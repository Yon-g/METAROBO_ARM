...
def save_position():  # 저장 기능
    respon = mc.get_angles()  # 로봇의 엔코더 값 가져오기
    commands.append(respon)  # 엔코더 값 저장
    codes.append(speed)  # 현재 속도 값을 복사하여 저장 (리스트로 저장)
    ...
    try:
        ...
        for i, cmd in enumerate(commands):
            if isinstance(cmd, str):
                ...
                mc.send_angles(cmd, codes[i])  # 해당 명령어에 맞는 속도 적용
    ...
    while move_complete:
        ...
        for i in range(len(target_joint)):
            ...
            if abs(target_joint[i] - cur_pos[i]) >= 15:
                ...
                mc.send_angles(target_joint,d_speed)
                time.sleep(0.1)
                good_for = True
                break
        if not good_for:
...
# "ALL Delete" 버튼
all_delete_button = tk.Button(root, text="전체 삭제", command=all_delete_command)
all_delete_button.grid(row=3, column=4, pady=10)
